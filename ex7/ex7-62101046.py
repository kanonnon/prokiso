...
def josephus(m, n):
    ...
    for _ in range(n-1):
        ...
        print(residents.pop(kill_index), end=" ") # 改行せずにprintする
        start_index = kill_index
    print(str(residents[0]) + " <= sit here!")
...
def generate_bracket_map(string):
    ...
    for i in range(len(string)):
        if string[i] == "[":
            stack.append(i) # stackリストにi（"["のインデックス）という要素を追加する
        elif string[i] == "]":
            j = stack.pop() # stackリストの一番後ろの要素を削除し、その値をjに代入する（対応する（"["のインデックス））
            bracket_map[i] = j # bracket_mapに、i（"]"のインデックス）をkey、j（"["のインデックス）をvalueとしてデータを入れる
            bracket_map[j] = i # bracket_mapに、j（"["のインデックス）をkey、i（"]"のインデックス）をvalueとしてデータを入れる
