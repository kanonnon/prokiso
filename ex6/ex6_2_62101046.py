...
def bubble_sort(num_array):
  ...
  check_sort = True # while文の条件をするための変数で、bool型が入る
  while check_sort: # check_sortがTrueの間ループを続ける（Falseになったら抜ける）
      check_sort = False # check_sortをFalseに初期化しておく
      for i in range(num_length - 1): # 前から2数字ずつ交換するから、配列の数-1組だけループする
          num_i = num_array[i] # 交換する数（左側）
          num_next = num_array[i+1] # 交換する数（右側）
          if num_i > num_next:
              check_sort = True # 交換が起きたら、check_sortをTrueにしてwhile文を抜けないようにする
