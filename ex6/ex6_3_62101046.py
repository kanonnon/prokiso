...
def open_field(field, x, y):
  ...
  if 0 <= x and x <= len(field[0])-1 and 0 <= y and y <= len(field)-1: # xとyがfieldの内側ならば
    if field[y][x] == 1: # 開けたマス目が地雷ならば
      return -1
    else: # 開けたマス目が地雷でないならば
      for i in range(x-1, x+2): # x軸に関するfor文（開けたマス目の横3列）
        for j in range(y-1, y+2): # y軸に関するfor文（開けたマス目の縦3列）
          # 開けたマス目がfieldの端っこの場合、周囲のマス目で存在しないところがあるから以下の条件分岐をする
          if 0 <= i and i <= len(field[0])-1 and 0 <= j and j <= len(field)-1: # 周囲のマス目がマス目として存在するなら
            if field[j][i] == 1: # とあるマス目が地雷ならば
              mine_count += 1 # mine_countに1を足す
      return mine_count
  else: # xとyがfieldの外側ならば
    return -2
